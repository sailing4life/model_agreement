...
# Keep list short; normalization handles spaces/degree symbols/variants
...
def _sniff_and_read(file: bytes) -> pd.DataFrame:
    """Robust reader: encoding + delimiter + EU decimals."""
    # Try encodings
    raw = None
...
def _auto_pick(colnames: List[str], candidates: List[str]) -> str | None:
    """Case-insensitive, symbol-tolerant column picker."""
    def norm(s: str) -> str:
        # lower + remove non [a-z0-9]
...
def standardize_columns(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, str]]:
    """Return df with added 'time','TWS','TWD' when possible plus a mapping to originals."""
...
mappings: Dict[str, Dict[str, str]] = {}  # keep original headers for xlabel
for f in uploaded:
...
# -------------------------
# Build a smart x-label from original CSV header
# -------------------------
