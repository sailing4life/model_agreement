# streamlit_app.py
# Meteogram Model Agreement — shared x, circular σ, header-based xlabel
...
def _auto_pick(colnames: List[str], candidates: List[str]) -> str | None:
    ...
    norm_map = {norm(c): c for c in colnames}
    ...
    for c in colnames:
        if any(ck in re.sub(r'[^a-z0-9]+','',c.lower()) for ck in cand_keys):
            return c
    return None
...
def standardize_columns(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    out = df.copy()
    if time_col:
        out["time"] = to_datetime_series(out, time_col); mapping["time"] = time_col
    if tws_col:
        out["TWS"]  = pd.to_numeric(out[tws_col], errors="coerce"); mapping["TWS"] = tws_col
    if twd_col:
        out["TWD"]  = pd.to_numeric(out[twd_col], errors="coerce"); mapping["TWD"] = twd_col
    if "time" in out:
...
def infer_common_freq(times: pd.Series) -> pd.Timedelta:
    if times.empty or len(times) < 3: return pd.Timedelta(hours=1)
...
def build_common_index(models: Dict[str,pd.DataFrame]) -> pd.DatetimeIndex:
    ...
    for _, df in models.items():
        ...
        t = df["time"]; freqs.append(infer_common_freq(t)); spans.append((t.min(), t.max()))
...
def regrid(df: pd.DataFrame, index: pd.DatetimeIndex) -> pd.DataFrame:
    if df.empty or "time" not in df: return pd.DataFrame(index=index)
    ...
    if "TWD" in out:
        rad = np.deg2rad(out["TWD"]); z = np.exp(1j*rad)
...
def circular_mean_deg(deg_values: np.ndarray) -> float:
    rad = np.deg2rad(deg_values); C = np.mean(np.cos(rad)); S = np.mean(np.sin(rad))
...
with st.sidebar:
    ...
    # FIXED: build the label string first, then pass it to checkbox
    band_label = f"Show within ±{band_val:g} {speed_unit} agreement" if band_val else "Show within ±band agreement"
...
if not uploaded:
    st.info("Upload two or more CSV files to begin."); st.stop()
...
for f in uploaded:
    ...
    try:
        ...
        models_raw[name] = std; mappings[name] = mapping
    except Exception as e:
        st.error(f"Failed to read {f.name}: {e}")

if not models_raw:
    st.error("No readable CSVs found."); st.stop()
...
if len(common_index)==0:
    st.error("Could not build a common time index."); st.stop()
...
if start_t > end_t:
    st.warning("Start after end; swapping."); start_t, end_t = end_t, start_t
...
if len(time_index)==0: time_index = common_index
...
if smooth:
    for df in models.values():
        ...
        if "TWD" in df:
            rad = np.deg2rad(df["TWD"]); z = np.exp(1j*rad)
...
if not frames_speed and not frames_dir:
    st.error("No TWS or TWD found."); st.stop()
...
if dir_agree_R is not None:
    ...
    dir_sigma_deg = np.rad2deg(dir_sigma_rad)

# X label from original header
time_headers = [m.get("time") for m in mappings.values() if m.get("time")]
...
with tab_speed:
    if not frames_speed:
        ...
        figS, (axS1, axS2) = plt.subplots(2, 1, sharex=True, figsize=(12, 8),
        ...
        if show_spread and std_speed is not None and mean_speed is not None:
            axS1.fill_between(mean_speed.index, (mean_speed-std_speed).values, (mean_speed+std_speed).values, alpha=0.15, label="±1σ")
        axS1.set_ylabel(f"Wind speed [{speed_unit}]"); axS1.grid(True, alpha=0.3); axS1.legend(ncols=3, fontsize=9)
        ...
        if has_any_speed_agree:
            if speed_agree_cv_pct is not None:
                axS2.plot(speed_agree_cv_pct.index, speed_agree_cv_pct.values, linewidth=1.8, label="Agreement (1−σ/μ) %")
            if show_band_agreement and speed_agree_band_pct is not None:
                axS2.plot(speed_agree_band_pct.index, speed_agree_band_pct.values, linewidth=1.8, label=f"Within ±{band_val:g} {speed_unit} %")
            axS2.set_ylim(0, 100); axS2.set_ylabel("Agreement [%]"); axS2.set_xlabel(time_xlabel)
            axS2.grid(True, alpha=0.3); axS2.legend()
        else:
